...
const Dashboard = () => {
  ...
  // GDPR rights: account-deletion modal + data-export state
  ...
  // Ambassador self-service data (only loaded when the user is an ambassador)
  ...
  useEffect(() => {
    if (user) {
      // Refresh the user record so role/ambassador changes made server-side
      // (e.g. an admin promoted this creator while their session was open)
      // become visible without re-login.
  ...
  // Pull referral data lazily the first time the Ambassador tab is opened
  // (or whenever the ambassador status changes).
  useEffect(() => {
    if (activeTab !== 'ambassador') return
    if (!user?.isAmbassador) return
    setReferralLoading(true)
    useDataStore.getState().fetchMyReferrals()
      .then(data => setReferralData(data))
      .catch(() => setReferralData(null))
      .finally(() => setReferralLoading(false))
  }, [activeTab, user?.isAmbassador])
  ...
  const copyReferralLink = async () => {
    ...
    try {
      ...
      // Clipboard blocked — fall back to selecting via prompt
  ...
  const handleDeleteAccount = async () => {
    ...
    setIsDeleting(true)
    const ok = await useAuthStore.getState().deleteAccount(deletePassword)
    setIsDeleting(false)
    if (ok) {
      navigate('/')
    } else {
      setDeleteError(useAuthStore.getState().error || 'Failed to delete account.')
    }
  }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setNewMessage('')
      // Refresh conversation list so a brand-new thread (e.g. with Fanvora
      // Support) shows up in the sidebar without a manual refresh.
      fetchConversations()
    } catch (error) {
      alert(error.message)
    }
  ...
  // Open (or create) a conversation with Fanvora Support. Idempotent — if a
  // thread already exists, this just selects it.
  const startSupportConversation = async () => {
    try {
      const support = await useDataStore.getState().fetchSupportInfo()
      setChatUser(support)
      const existing = await useDataStore.getState().fetchMessages(support.id)
      setMessages(existing)
    } catch (err) {
      alert(err.message || 'Could not open Fanvora Support')
    }
  }
  ...
  const navItems = isCreator
    ? [
        ...
        // Ambassador tab is only shown to creators with the ambassador role.
        ...(user?.isAmbassador ? [{ id: 'ambassador', icon: Award, label: 'Ambassador' }] : []),
    ...
    <div className="min-h-screen bg-dark-bg">
      ...
      <div className="flex pt-16">
        ...
        <main className="flex-1 lg:ml-64 p-4 lg:p-8">
                  ...
                  <div className="w-80 border-r border-dark-border overflow-y-auto">
                    <div className="p-4 border-b border-dark-border space-y-3">
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-primary/10 hover:bg-primary/20 border border-primary/30 transition-colors text-left"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-semibold text-white">Message Fanvora Support</div>
                          <div className="text-xs text-gray-400 truncate">
                            Trust &amp; safety, billing, account help
                          </div>
                        </div>
                      </button>
                    </div>
          ...
          {/* Ambassador Tab — visible only to creators promoted to ambassador */}
          {activeTab === 'ambassador' && isCreator && user?.isAmbassador && (
            <div className="space-y-6">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-gray-400 text-sm">
                    Earn {((referralData?.commissionRate ?? 0.05) * 100).toFixed(1)}% commission
                    on every transaction by creators who join through your link.
                  </p>
                </div>
              </div>
              ...
              <div className="glass rounded-2xl p-6 border border-accent/30">
                ...
                <div className="flex flex-col md:flex-row gap-2">
                  ...
                  <button
                    ...
                    className="btn-primary inline-flex items-center justify-center gap-2 min-w-[120px] disabled:opacity-50"
                  >
                ...
                <p className="text-xs text-gray-500 mt-3">
                  Anyone who registers through this link will be permanently linked to you. The
                  5% commission comes from Fanvora's 30% platform cut, so your referrals still
                  get their full 70% creator share.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="glass rounded-2xl p-5">
                  <div className="text-xs text-gray-400 mb-2">Total commission earned</div>
                  <div className="font-accent text-2xl font-bold text-green-400">
                    {formatNaira(referralData?.totalCommission || 0)}
                  </div>
                </div>
                <div className="glass rounded-2xl p-5">
                  <div className="text-xs text-gray-400 mb-2">Active referrals</div>
                  <div className="font-accent text-2xl font-bold">
                    {referralData?.activeReferredCount ?? 0}
                ...
                <div className="glass rounded-2xl p-5">
                  <div className="text-xs text-gray-400 mb-2">Payouts received</div>
                  <div className="font-accent text-2xl font-bold">
                    {referralData?.payoutCount ?? 0}
                  </div>
                </div>
                <div className="glass rounded-2xl p-5">
                  <div className="text-xs text-gray-400 mb-2">Commission rate</div>
                  <div className="font-accent text-2xl font-bold text-accent">
                    {((referralData?.commissionRate ?? 0.05) * 100).toFixed(1)}%
                  </div>
                </div>
              </div>
              ...
              {/* Referred creators */}
              <div>
                <h2 className="font-heading text-lg font-semibold mb-3">Creators you referred</h2>
                <div className="glass rounded-2xl overflow-hidden">
                  {referralLoading ? (
                    <div className="p-12 text-center text-gray-500">
                      <Loader2 className="w-5 h-5 animate-spin inline mr-2" /> Loading…
                    </div>
                  ) : (referralData?.referredCreators?.length || 0) === 0 ? (
                    <div className="p-12 text-center">
                      <Award className="w-10 h-10 text-gray-600 mx-auto mb-3" />
                      <p className="text-gray-400 text-sm mb-1">No referrals yet</p>
                      <p className="text-gray-500 text-xs">
                        Share your link with creators in your network to start earning.
                      </p>
                    </div>
                  ) : (
                    <div className="divide-y divide-dark-border">
                      {referralData.referredCreators.map((c) => (
                        <div key={c.id} className="p-4 flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            <img
                              src={c.avatar || `https://ui-avatars.com/api/?name=${c.displayName}&background=5A0F4D&color=fff`}
                              alt=""
                              className="w-10 h-10 rounded-full object-cover flex-shrink-0"
                            />
                            <div className="min-w-0">
                              <Link to={`/creator/${c.username}`} className="font-semibold hover:text-primary truncate block">
                                {c.displayName}
                              </Link>
                              <div className="text-xs text-gray-500 truncate">
                                @{c.username} · joined {new Date(c.joinedAt).toLocaleDateString()}
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {c.isSuspended && <span className="badge bg-red-500/20 text-red-400">Suspended</span>}
                            {c.isDeleted && <span className="badge bg-gray-500/20 text-gray-400">Deleted</span>}
                            {!c.isSuspended && !c.isDeleted && <span className="badge bg-green-500/20 text-green-400">Active</span>}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div>
                <h2 className="font-heading text-lg font-semibold mb-3">Recent commission payouts</h2>
                <div className="glass rounded-2xl overflow-hidden">
                  {referralLoading ? (
                    <div className="p-12 text-center text-gray-500">
                      <Loader2 className="w-5 h-5 animate-spin inline mr-2" /> Loading…
                    </div>
                  ) : (referralData?.payouts?.length || 0) === 0 ? (
                    <div className="p-12 text-center">
                      <DollarSign className="w-10 h-10 text-gray-600 mx-auto mb-3" />
                      <p className="text-gray-400 text-sm mb-1">No payouts yet</p>
                      <p className="text-gray-500 text-xs">
                        Commission accrues as your referrals earn revenue.
                      </p>
                    </div>
                  ) : (
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              <div className="glass rounded-2xl p-6">
                ...
                <p className="text-gray-400 text-sm mb-4">
                  Under GDPR and Nigeria's NDPA you have the right to access a copy of
                  your personal data in a machine-readable format. Click below to download
                  everything Fanvora holds about you as a JSON file.
                </p>
                {exportError && (
                  <div className="flex items-center gap-2 p-3 mb-3 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-sm">
              ...
              <div className="glass rounded-2xl p-6 border border-red-500/30">
                ...
                <p className="text-gray-400 text-sm mb-4">
                  Permanently delete your account. Your profile, posts, and likes will be
                  removed; subscriptions you have or that fans hold to you will be
                  cancelled; messages you sent are anonymised. Financial records are
                  retained for 7 years per Nigerian tax and AML law but are no longer
                  linked to your identity. This action cannot be undone.
                </p>
                <button
                  onClick={() => {
                    setDeleteError('')
                  ...
                  className="bg-red-500/20 hover:bg-red-500/30 text-red-400 font-semibold py-3 px-6 rounded-xl transition-colors"
                >
                  Delete Account
                </button>
              </div>
          ...
          <div className="modal-content max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-red-400">
                ...
                <h2 className="font-heading text-xl font-bold">Delete account</h2>
              </div>
              {!isDeleting && (
                <button
            ...
            <p className="text-gray-300 text-sm mb-6">
              This will <strong>permanently delete</strong> your profile, posts, and
              likes; cancel your subscriptions; and anonymise your messages. We cannot
              recover the account once deleted. To confirm, enter your password and type
              <strong> DELETE</strong> below.
            </p>

            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-400 mb-2">Your password</label>
                <input
                  type="password"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 bg-red-500 hover:bg-red-600 text-white font-semibold py-3 px-6 rounded-xl transition-colors disabled:opacity-60 inline-flex items-center justify-center gap-2"
                >
