...
const CreatorProfile = () => {
  ...
  const [reportTxRef, setReportTxRef] = useState('')
  ...
  const handlePurchase = async (postId) => {
    ...
    try {
      const result = await purchasePost(postId)
      if (result?.authorizationUrl) {
        window.location.href = result.authorizationUrl
      } else {
        alert('Could not start payment — no authorization URL returned')
  ...
  const handleReportSubmit = async () => {
    ...
    if (!reportSummary.trim() || reportSummary.trim().length < 10) {
      setReportError('Please describe the issue (at least 10 characters).')
      return
    }
    setReportSubmitting(true)
    try {
      await useDataStore.getState().createReport({
        againstUserId: creator.id,
        type: reportType,
        summary: reportSummary.trim(),
        transactionRef: reportTxRef.trim() || null
      })
  ...
  return (
    <div className="min-h-screen bg-dark-bg">
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-6 -mt-20 relative">
            ...
            <div className="flex items-center gap-3 pb-4 flex-wrap">
              ...
              <button className="btn-secondary p-3" aria-label="Share profile">
              ...
              {!isOwnProfile && (
                <button
                  ...
                  className="btn-secondary p-3 text-red-300 hover:text-red-400"
                  aria-label="Report this creator"
              ...
              {!isOwnProfile && (
                <button onClick={handleSubscribe} className="btn-primary">
                  {isSubscribed ? 'Subscribed' : `Subscribe ${formatNaira(creator.subscriptionPrice)}/mo`}
                </button>
              )}
            </div>
          </div>
            ...
            <div className="glass rounded-2xl p-6 sticky top-24">
              <div className="text-center mb-6">
                <div className="font-accent text-4xl font-bold gradient-text">
                  {formatNaira(creator.subscriptionPrice)}
                </div>
                          ...
                          >
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-bg/80">
                              ...
                              <p className="text-sm text-gray-400">
                                Purchase for {formatNaira(post.ppvPrice)}
                              </p>
                              {isSubscribed && (
                ...
                {[1000, 2500, 5000, 10000].map((amount) => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg transition-colors ${
                      Number(tipAmount) === amount ? 'bg-primary' : 'bg-dark-border hover:bg-primary/30'
                    }`}
                  >
                    {formatNaira(amount)}
                  </button>
      ...
      {/* Report modal — files a moderation case visible to admins. The reporter
          gets an automatic acknowledgment DM from Fanvora Support, and any
          resolution will arrive in the same thread. */}
      {showReportModal && creator && (
        <div className="modal-overlay" onClick={() => !reportSubmitting && closeReportModal()}>
          <div className="modal-content max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-red-400">
                <Flag className="w-5 h-5" />
                <h2 className="font-heading text-xl font-bold">Report @{creator.username}</h2>
              </div>
              {!reportSubmitting && (
                <button
            ...
            {reportSuccess ? (
              <div className="text-center py-6">
                ...
                <h3 className="font-heading text-lg font-bold mb-2">Report received</h3>
                <p className="text-sm text-gray-400 mb-6">
                  Our trust &amp; safety team will review your report. Check{' '}
                  <Link to="/dashboard" className="text-primary hover:text-primary-light">
                    Messages
                  </Link>{' '}
                  for an acknowledgment from Fanvora Support — any resolution will arrive
                  in the same thread.
                </p>
                <button onClick={closeReportModal} className="btn-primary w-full">Done</button>
              </div>
            ) : (
              <>
                <p className="text-sm text-gray-400 mb-4">
                  Help us keep Fanvora safe. Tell us what happened and we'll get back to
                  you on Messages.
                </p>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="conduct">Harassment / abusive conduct</option>
                      <option value="spam">Spam or scam</option>
                      <option value="impersonation">Impersonation</option>
                      <option value="copyright">Copyright / IP infringement</option>
                      <option value="refund">Payment / refund dispute</option>
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">
                      What happened? <span className="text-gray-600">(min 10 characters)</span>
                    </label>
                    <textarea
                      value={reportSummary}
                      onChange={(e) => setReportSummary(e.target.value)}
                      className="input-field h-32 resize-none"
                      placeholder="Describe the issue in your own words. Include dates, post URLs, or message references where helpful."
                      maxLength={2000}
                      disabled={reportSubmitting}
                    />
                    <div className="text-xs text-gray-500 mt-1 text-right">
                      {reportSummary.length}/2000
                    </div>
                  </div>
                  ...
                  {reportType === 'refund' && (
                    <div>
                      <label className="block text-sm text-gray-400 mb-2">
                        Transaction reference <span className="text-gray-600">(optional)</span>
                      </label>
                      <input
                        type="text"
                        value={reportTxRef}
                        onChange={(e) => setReportTxRef(e.target.value)}
                        className="input-field"
                        placeholder="e.g. sub_abc123… or tip_xyz456…"
                        disabled={reportSubmitting}
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Find this on your Paystack receipt email.
                      </p>
                    </div>
                  )}
