...
import { Mail, Lock, Eye, EyeOff, User, AlertCircle, Check, Award } from 'lucide-react'
...
const Register = () => {
  ...
  // If a ?ref= code is in the URL, look up the ambassador so we can show
  // "Referred by @username" in the form.
  useEffect(() => {
    if (!referralCode) return
    let cancelled = false
    apiFetch(`/auth/referral/${encodeURIComponent(referralCode)}`)
      .then(data => { if (!cancelled) setReferrer(data.ambassador) })
      .catch(() => {})
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        ...
        {referrer && (
          <div className="mb-4 p-4 rounded-xl bg-accent/10 border border-accent/30 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-accent/20 flex items-center justify-center flex-shrink-0">
              <Award className="w-5 h-5 text-accent" />
            </div>
            <div className="text-sm">
              <div className="text-white font-semibold">Referred by @{referrer.username}</div>
              <div className="text-gray-400 text-xs">
                You'll keep your full 70% creator share. The 5% ambassador commission comes
                out of the platform's cut, not yours.
              </div>
            </div>
          </div>
        )}
