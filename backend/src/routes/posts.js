...
// Purchase post — initializes a real PayStack transaction. The actual
// purchase record is created on /api/payments/verify (or via webhook)
// after PayStack confirms the charge succeeded.
router.post('/:id/purchase', authenticate, async (req, res) => {
  try {
    ...
    if (hasPurchased) return res.status(400).json({ message: 'You already own this content' })

    const reference = `ppv_${req.user.id}_${post.id}_${Date.now()}`
    const data = await paystack.initializeTransaction({
      email: req.user.email,
      amountNgn: post.ppvPrice,
      reference,
      callbackUrl: callbackUrl(),
      metadata: { type: 'ppv', fanId: req.user.id, postId: post.id }
    })
    ...
    console.error('Purchase post error:', err.message, err.paystack || '')
    const status = err.status && err.status >= 400 && err.status < 600 ? err.status : 502
    res.status(status).json({
      message: err.message || 'Failed to initialize PPV payment',
